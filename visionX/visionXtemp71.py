...
#HughCircles Detection TEST  
    try:
        circles = cv2.HoughCircles(gray,cv2.HOUGH_GRADIENT,1,100,param1=30,param2=40,
        ...
        for cnt in contours:
            ...
            if area>400:
                ...
                if(cx>40 and cx<290 and cy>450 and cy<660 ):
                    ...
                    cv2.line(frame,(cx,cy),2,(0,0,255),-1)
...
def contour_circles(frame):
    ...
    cv2.createTrackbar("L-H","Trackbars",45,180,nothing)
    cv2.createTrackbar("L-S","Trackbars",47,255,nothing)
    cv2.createTrackbar("L-V","Trackbars",46,255,nothing)
    cv2.createTrackbar("U-H","Trackbars",107,180,nothing)
    cv2.createTrackbar("U-S","Trackbars",193,255,nothing)
    ...
    try:
        for cnt in contours:
            ...
            if area>10:
                ...
                elif 10<len(approx)<5000:
                    cv2.putText(frame,"Circle",(x,y),font,1,(0,0,0))

                
            cv2.line(frame,(cx,cy),(x,y),(0,255,0),1)
            cv2.line(frame,(x,int(y/2)),(x,y),(0,0,0),1)
...
while(True):
    ...
    if ret==True:
        ...
        print(shapeflag)
        newtime=time.time()
        #if flagard==None and shapeflag==0 and newtime-oldtime>=2:
            # oldtime=time.time()
            # arduino('D')
            # time.sleep(0.5)
            # ret,frame=cap.read()
