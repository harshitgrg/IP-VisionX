...
def shape2():
    frameC=frame
    lt=np.array([0,20,50])
    ut=np.array([140,190,200])
    maskC=cv2.inRange(frameC,lt,ut)
    kernelC=np.ones((3,3),np.uint8)

    maskC=cv2.erode(maskC,kernelC)
    contoursC, _ = cv2.findContours(maskC,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
    cv2.imshow('maskC',maskC)
    for cnt in contoursC:
        area=cv2.contourArea(cnt)
        approx=cv2.approxPolyDP(cnt,0.01*cv2.arcLength(cnt,True),True)
        x=approx.ravel()[0]
        y=approx.ravel()[1]
        #if area>10:
        cv2.drawContours(maskC,[approx],-1,(70,255,150),2)

        if len(approx) == 3:
            cv2.putText(maskC, "Triangle", (4,4), cv2.FONT_HERSHEY_COMPLEX, 3, (0, 255, 0))
            
        elif len(approx) == 4:
            
            cv2.putText(maskC, "square", (x, y), cv2.FONT_HERSHEY_COMPLEX, 3, (0, 0, 255))
...
cap=cv2.VideoCapture("r1_toptrack.mp4")
