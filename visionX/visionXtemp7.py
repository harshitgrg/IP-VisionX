...
#HughCircles Detection TEST  
    try:
        circles = cv2.HoughCircles(gray,cv2.HOUGH_GRADIENT,1,100,
                            param1=45,param2=20,minRadius=80,maxRadius=230) 
        circles = np.uint16(np.around(circles))
        ret,thresh = cv2.threshold(gray,127,255,0)
        ...
        if circles.all != None:
            ...
            for i in circles[0,:]:
                if i[1]==ar[len(ar)-1]:
                    ...
                    if theta1>5:
                        #dist=(theta1*11.5/2)
                        print('theta for turn right '+str(theta1))
                        ...
                        camerac=29/750 #665 is length of y axis
                        anglec=360/(2*np.pi*5)
                        stepsc=16/1.8
                        #print(distancecm)
    ...
    try:
        ...
        for cnt in contours:
            ...
            if(cx>40 and cx<290 and cy>450 and cy<660 ):
                ...
                cv2.line(frame,(x,y),(x,int(y/2)),(50,70,100))
